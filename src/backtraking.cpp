...
class GraphDfsHeur : public GraphInterface {
    public:
        ...
        GraphDfsHeur(int cols, int rows, int x_size, int y_size){
            this->cols = cols;
            this->x_size = x_size;
            this->rows = rows;
            this->y_size = y_size;
...
std::pair<int,int> GraphDfsHeur::getDirection(std::pair<int, int> curr_direction, std::vector<std::pair<int,int>> possible_directions){
...
int main(int argc, char **argv){
    srand(time(NULL));
    GraphDfsHeur maze = GraphDfsHeur(20, 20, 400, 400);
    maze.initMaze();
    maze.createMaze();
    maze.printMaze();
}
